 ...
 */
extern bool rustsimplicity_0_7_bitcoin_execSimplicity( simplicity_err* error, unsigned char* ihr
  ...
  rustsimplicity_0_7_assert(NULL != error);
  rustsimplicity_0_7_assert(NULL != tx);
  rustsimplicity_0_7_assert(NULL != taproot);
  rustsimplicity_0_7_assert(0 <= minCost);
  rustsimplicity_0_7_assert(minCost <= budget);
  rustsimplicity_0_7_assert(NULL != program || 0 == program_len);
  rustsimplicity_0_7_assert(NULL != witness || 0 == witness_len);
...

  {
    ...
    dag_len = rustsimplicity_0_7_decodeMallocDag(&dag, rustsimplicity_0_7_bitcoin_decodeJet, &census, &stream);
    if (dag_len <= 0) {
      rustsimplicity_0_7_assert(dag_len < 0);
      *error = (simplicity_err)dag_len;
      return IS_PERMANENT(*error);
    }
    rustsimplicity_0_7_assert(NULL != dag);
    rustsimplicity_0_7_assert((uint_fast32_t)dag_len <= DAG_LEN_MAX);
    *error = rustsimplicity_0_7_closeBitstream(&stream);
  }
  ...
  if (IS_OK(*error)) {
    type* type_dag = NULL;
    *error = rustsimplicity_0_7_mallocTypeInference(&type_dag, rustsimplicity_0_7_bitcoin_mallocBoundVars, dag, (uint_fast32_t)dag_len, &census);
    if (IS_OK(*error)) {
      rustsimplicity_0_7_assert(NULL != type_dag);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_7_fillWitnessData(dag, type_dag, (uint_fast32_t)dag_len, &witness_stream);
      if (IS_OK(*error)) {
        *error = rustsimplicity_0_7_closeBitstream(&witness_stream);
    ...
    if (IS_OK(*error)) {
      ...
      *error = rustsimplicity_0_7_verifyNoDuplicateIdentityHashes(&ihr_buf, dag, type_dag, (uint_fast32_t)dag_len);
    ...
    if (IS_OK(*error) && amr) {
      ...
      analyses *analysis = rustsimplicity_0_7_malloc((size_t)dag_len * sizeof(analyses));
      if (analysis) {
        rustsimplicity_0_7_computeAnnotatedMerkleRoot(analysis, dag, type_dag, (uint_fast32_t)dag_len);
      ...
      rustsimplicity_0_7_free(analysis);
    }
    if (IS_OK(*error)) {
      txEnv env = rustsimplicity_0_7_bitcoin_build_txEnv(tx, taproot, ix);
    ...
    rustsimplicity_0_7_free(type_dag);
  }

  rustsimplicity_0_7_free(dag);
  return IS_PERMANENT(*error);
}
